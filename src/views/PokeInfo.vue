<template>
  <div class="flex flex-col h-screen">
    ...
    <div class="flex flex-1 items-center justify-center">
      <!-- COMPONENTE DE ERROR -->
      <errorComponent v-if="errorStatus && !loading" />
      <!-- LOADING COMPONENT -->
        ...
        <div class="mx-auto max-w-2xl px-6 lg:max-w-full lg:px-8">
          ...
          <div class="mt-6 grid grid-cols-1 gap-4 sm:mt-8 lg:grid-cols-3 lg:grid-rows-1">
              ...
              <div class="relative flex h-full flex-col overflow-auto rounded-[calc(theme(borderRadius.lg)+1px)] lg:rounded-l-[calc(2rem+1px)]">
                <div class="flex justify-center px-8 pb-3 pt-8 sm:px-8 sm:pb-0 sm:pt-10">
                  <div v-for="(type, id) in pokemon.types" :key="id">
                    <img class="mx-3" :src="type.imgUrl" :alt="type.name" />
                  </div>
                </div>
                <div class="relative min-h-[30rem] w-full grow [container-type:inline-size] max-lg:mx-auto max-lg:max-w-sm">
                  <img class="object-top py-6 sm:size-full lg:p-12" :src="pokemon.image" :alt="pokemon.name" />
                </div>
              </div>
              <div
            ...
            <div class="relative">
              ...
              >
                ...
                <div class="flex justify-center flex-wrap px-1 pb-3 pt-2 mb-4 sm:px-1 sm:pb-0 sm:pt-2">
                  <div v-for="(enemyType, id) in pokemon.matchUps" :key="id">
                    <img class="m-1 w-[208px] h-[48px]" :src="enemyType.imgUrl" v-if="enemyType.score > 0" :alt="enemyType.name" />
                  </div>
                </div>
              ...
              <div
                class="pointer-events-none absolute inset-px rounded-lg shadow-xl ring-2 ring-[var(--color-contrast)] max-lg:rounded-t-[2rem]"
              ></div>
            </div>
...
const pokemon = ref<Pokemon>({ id: 0, name: '', image: '', stats: [], types: [], matchUps: [] });
...
const fetchTypesImg = () => {
  if (!pokemon.value.matchUps) return;
  for (const type of pokemon.value.matchUps) {
    PokemonSvc.getTypeImg(type.name).then((res) => {
      type.imgUrl = res;
    });
  }
};
...
watch(
  () => route.params.id,
  (newId) => {
    if (newId !== undefined && newId !== null && newId) fetchPokemon(newId);
...
const searchPokemon = (searchInput: any) => {
  searchInput = searchInput.toLowerCase();
  router.push({ name: 'Info', params: { id: searchInput } });
};
